    ...
    return (
        ...
        <nav className={`fixed top-0 left-0  w-full flex items-center justify-between px-4 md:px-16 lg:px-24 xl:px-32 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
            ...
            <div className="hidden md:flex items-center gap-4">
                ...
                {user ?
                    (<div className="flex items-center gap-3">
                        <button onClick={() => navigate('')} className={`border px-4 py-1 text-sm font-light rounded-full cursor-pointer ${isScrolled ? 'text-black' : 'text-white'} transition-all`} >
                            Become a host
                        </button>
                        <UserButton>
            ...
            <div className="flex items-center gap-3 md:hidden">
                ...
                <img onClick={() => setIsMenuOpen(!isMenuOpen)} src={assets.menuIcon} alt="menu" className={`${isScrolled && 'invert'} h-4 cursor-pointer hover:animate-bouncing`} />
            </div>
